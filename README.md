...
The prayer page fills itself in from Cloudflare's visitor location headers when the zone sends them (`cf-ipcity`, `cf-iplatitude`, `cf-iplongitude`, `cf-iptimezone` — the "Add visitor location headers" managed transform). That costs nothing and asks for no permission; it is city-level and can be wrong behind a VPN, so it pre-fills the form rather than replacing what is shown.
