...
- `GOOGLE_MAPS_API_KEY` — optional, server-side only. Names the place behind the visitor's granted coordinates and covers cities the free geocoder misses. The browser never receives it. Set it in the site's `.env`, which the app reads at runtime.

Nothing about the visitor may reach the HTML: one cached copy is served to everyone, so the pages carry a fixed zone and a fixed city and the browser corrects both after hydration. The clock and date follow the visitor's own device zone (`Intl.DateTimeFormat().resolvedOptions().timeZone`) unless they have chosen one, which is remembered in `localStorage` and wins. The prayer page asks `/api/location` once on load and recomputes for the place it learns: coordinates when Cloudflare's visitor location headers are enabled for the zone (`cf-ipcity`, `cf-iplatitude`, `cf-iplongitude`, `cf-iptimezone` — the "Add visitor location headers" managed transform), otherwise the curated city for `cf-ipcountry`. It costs nothing and asks for no permission; it is connection-level and can be wrong behind a VPN, so the result says it is an estimate and the form stays editable.
