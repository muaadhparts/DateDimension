 ...
 */
export default function AppShell({lang, page, citySlug, zone, year, children}: {
  lang: Lang;
  page: string;
  citySlug?: string;
  zone: string;
  ...
  return <div lang={lang} dir={ar?'rtl':'ltr'} className="shell"><a className="skip" href="#main">{t('انتقل إلى المحتوى','Skip to content')}</a><header><a className="brand" href={href()}><Clock3 size={29}/><span>{t('يومك الآن','Your Day Now')}<span className="sub" style={{display:'block',fontSize:11,letterSpacing:3}}>YOUR DAY NOW</span></span></a><nav aria-label={t('التنقل الرئيسي','Main navigation')}>{NAV_ROUTES.map((r)=><a key={r.key} className={page===r.key?'active':''} aria-current={page===r.key?'page':undefined} href={href(r.key)}>{pick(lang, r.nav)}</a>)}</nav><a className="language" href={`/${ar?'en':'ar'}${page?'/'+page:''}${citySlug?'/'+citySlug:''}`} hrefLang={ar?'en':'ar'}>{ar?'English':'العربية'}</a></header>
 <main id="main">{page&&<div className="breadcrumb"><a href={href()}>{t('الرئيسية','Home')}</a> / {title}</div>}<div className="intro"><div><div className="eyebrow">{t('كل يوم، على توقيتك','EVERY DAY, IN YOUR TIME')}</div><h1>{title}{city ? ' · ' + (ar ? city.ar : city.en) : ''}</h1><p>{route ? pick(lang, route.intro) : ''}</p></div>{page===''&&<span className="tag"><Globe2 size={14}/>{zone}</span>}</div>
    {children}
