    ...
    <div className="quran-artwork">
      ...
      {state === 'error' ? (
        <div className="artwork-error" role="status">
          <p>
            {t(
              'تعذّر تحميل صورة المصحف. يمكنك قراءة النص أسفل الصفحة أو إعادة المحاولة.',
              'The mushaf image could not load. Read the text below or try again.',
            )}
          </p>
          <button
            type="button"
        ...
        <div
          className={`artwork-scroll${zoom ? ' is-enlarged' : ''}`}
          tabIndex={zoom ? 0 : undefined}
          role="region"
          aria-label={t(`صورة صفحة ${page} من المصحف`, `Mushaf page ${page} image`)}
          aria-busy={state === 'loading'}
        >
          {state === 'loading' && (
            <span className="artwork-loading" role="status">
              {t('جارٍ تحميل صفحة المصحف…', 'Loading mushaf page…')}
            </span>
          )}
          ...
          <img
            ref={imageRef}
            key={attempt}
            src={`${artworkUrl(page)}&retry=${attempt}`}
            width={345}
            height={550}
            alt={t(
              `صفحة ${page} من مصحف المدينة بالرسم العثماني`,
              `Page ${page} of the Madinah mushaf in Uthmani script`,
            )}
      ...
      {zoom && (
        <p className="sub">
          {t(
            'حرّك الصفحة أفقيًا لقراءة السطر كاملًا.',
            'Scroll horizontally to read the full line.',
          )}
        </p>
      )}
