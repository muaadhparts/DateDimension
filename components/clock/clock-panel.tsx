...
export default function ClockPanel({
  ar, zone, onZoneChange, ZonePicker,
}: {
  ar: boolean;
  zone: string;
  onZoneChange: (zone: string) => void;
  ZonePicker: (props: {value: string; onChange: (v: string) => void; items: [string, string][]; label: string}) => React.ReactElement;
}) {
  ...
  return (
    <section className="panel clock-panel">
        ...
        <div className="digital" aria-label={t('الوقت الحالي', 'Current time')}>{reading}</div>
      ...
      <div className="status sub" role="status">{status}</div>
    </section>
  );
}
