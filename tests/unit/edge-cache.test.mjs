...
test('Every route class gets a policy, and unknown paths get none', () => {
  ...
  assert.equal(
    policyFor('/ar/about', now).sMaxAge,
    3600,
    'a dateless page, capped so a deploy lands',
  );
  assert.equal(policyFor('/ar/quran', now).sMaxAge, 3600, 'the surah index does not change');
  assert.equal(policyFor('/ar/quran/18', now).sMaxAge, 3600, 'nor does a surah');
  assert.equal(policyFor('/ar/mushaf/1', now).sMaxAge, 3600, 'nor a page of the mushaf');
